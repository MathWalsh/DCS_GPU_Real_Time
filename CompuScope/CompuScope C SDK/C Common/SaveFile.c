...
int32 CS_SUPPORT_API CsAs_SaveFile(LPCTSTR szFileName, void* pBuffer, const int sType, FileHeaderStruct *pHeader)
{
	...
	else if ( TYPE_BIN == sType || TYPE_BIN_APPEND == sType)
	{
		__int64 i64WriteSizeInBytes = pHeader->i64Length * pHeader->u32SampleSize;
		// We use fwrite()
		// But there is a bug in C runtime library fwrite(): it cannot write bigger than 4GB at one shot even with 64-bit app
		// Limite the write size 
		const unsigned long c_ulMaxChunkSize = (UINT_MAX);		// avoid reach 4G memory for each write

		// Write the file in chunks 
		size_t t_WriteSize	= c_ulMaxChunkSize;
		char* pBuffer2Write = (char*)pBuffer;
		...
		{
			if (i64WriteSizeInBytes < c_ulMaxChunkSize)
				t_WriteSize = (size_t) i64WriteSizeInBytes;

			fwrite(pBuffer2Write, 1, t_WriteSize, pFile );
			i64WriteSizeInBytes -= t_WriteSize;
			pBuffer2Write		+= t_WriteSize;
		}
		fclose(pFile);
		return CS_SUCCESS;
	}
